...
- **Synthesizer agent** (`src/agents/synthesizer_agent.py`) — `synthesize_response(question, sql_result=None, diagnostic_result=None, errors=None) -> str`. Also deterministic template assembly rather than an LLM call: renders only the Markdown sections for routes that actually produced a result, falls back to a "No results available" message when neither did, and runs a final `scrub_text()` pass over the assembled output. Not yet called from anywhere.
