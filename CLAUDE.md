...
- **Router agent** (`src/agents/router_agent.py`) — `route_query(question) -> list[Literal["sql", "diagnostic"]]` (empty list = unsupported). Deliberately not an LLM agent: deterministic word-boundary keyword regexes, per the POC decision to defer an LLM router. Returns a *route set*, not a single route, so a mixed-intent question (e.g. "why are there so many errors, show me the top apps") can fan out to both agents — routes are ordered diagnostic-before-sql, and `synthesizer_agent.py` relies on that order. Not yet called from anywhere (no graph exists yet — see Phase 3 in `TODO.md`, which now documents the fan-out this implies for the graph's conditional edges).
- **Synthesizer agent** (`src/agents/synthesizer_agent.py`) — `synthesize_response(question, sql_result=None, diagnostic_result=None, errors=None) -> str`. Deterministic template assembly for every case except one: when both a SQL and a diagnostic result are present, `get_agent_model("synthesizer_agent")` is used instead to weave them into one narrative rather than concatenating two disjoint sections (falls back to the template if that call raises). Runs a final `scrub_text()` pass over the output either way. Not yet called from anywhere.
- **DuckDB engine** (`src/engines/`) and **PII scrubber** (`src/security/pii_scrubber.py`).

Not implemented (exists only as empty file, missing entirely, or stubbed in README): `security_agent.py` (empty), `src/graph/state.py` / `workflow.py` (LangGraph orchestration is not wired up — agents are called directly), RBAC, Presidio/NER-based PII, non-DuckDB engine adapters.
...
**PII scrubbing runs inside the engine, not just at the agent boundary**: `DuckDBEngine._execute_query` calls `scrub_dataframe()` on every query result before returning it (`src/engines/duckdb_engine.py:49`), and `query_log_agent_with_retry` also calls `scrub_text()` on the user's prompt before it reaches the agent (`src/agents/sql_agent.py:66`). `diagnose_log_failure` follows the same input/output pattern, scrubbing both the incoming log payload and the agent's final analysis (`src/agents/diagnostic_agent.py`). `synthesize_response` adds one more layer at the very end of the pipeline, scrubbing the fully assembled Markdown before it's returned (`src/agents/synthesizer_agent.py`). Any new query/agent path should preserve these scrub points rather than relying on just one.
