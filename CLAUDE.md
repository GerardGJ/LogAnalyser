...
- **Diagnostic agent** (`src/agents/diagnostic_agent.py`) — root-cause analysis over stack traces/log snippets. Note: its model is currently hard-coded (`init_chat_model("openai:gpt-5.5")`) rather than read from `config/agents.yaml` like the SQL agent — don't assume config-driven model selection is universal until this is fixed.
